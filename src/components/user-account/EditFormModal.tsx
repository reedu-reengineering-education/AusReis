// //  Path: src/components/user-account/EditFormModal.tsx
// // Component EditFormModal is a form that allows users to edit an expense. The form includes fields for the expense amount, description, project, and category. Users can also upload files as proof of the expense. The form includes a calendar for selecting travel dates if the expense category is "travel". The form is submitted to the onUpdate function when the user clicks the "Aktualisieren" button. The form also includes a "Abbrechen" button to close the modal. The form displays a loader while submitting the form. The form also displays a success message when the form is successfully submitted.
// "use client";
...
export function EditFormModal({ expense, onUpdate }: EditFormModalProps) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "grossAmount") {
      ...
      const netAmount = calculateNetAmount(grossAmount, formData.category);
      setFormData((prevData) => ({
        ...prevData,
        grossAmount,
        netAmount,
      ...
      setFormData((prevData) => ({
        ...prevData,
        [name]: name === "netAmount" ? Number(value) : value,
      }));
    }
  };
  ...
  const calculateNetAmount = (
    ...
    const taxRate = category === "reimbursement" ? 0.19 : 0.07;
    return Number((grossAmount / (1 + taxRate)).toFixed(2));
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="grossAmount">Bruttobetrag</Label>
                <Input
                  id="grossAmount"
                  name="grossAmount"
